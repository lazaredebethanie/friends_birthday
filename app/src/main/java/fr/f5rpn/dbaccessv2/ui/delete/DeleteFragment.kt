...
class DeleteFragment : Fragment(), AnkoLogger {
    ...
    private lateinit var btnDeleteLoc: Button
    ...
    override fun onCreateView(
        ...
        btnDeleteLoc = root.findViewById(R.id.btnCommonDelete)
            ...
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                if (parent?.getItemAtPosition(position) == "Choose a name") {
                    ...
                    if (position!=0) {
                        ...
                        txtIdHidden.setText(listNom[myListId[position]].idPerson.toString())
    ...
    fun delete () {
        ...
        val age = txtBirthdayLoc.text.toString()
        val id = txtIdHiddenLoc.text.toString()
        val ctx = requireContext()
        val sqlLiteCallDB by lazy { SqlLiteCallDB(SqlLiteHelper(ctx)) }
        var person = Person(id.toInt(), name, firstName, age, 99)
        var rc = sqlLiteCallDB.updatePerson(person)
        info("Return code after insert in database : " + rc.toString())
        if (rc > 0) {
            //longToast("Friend modified !")
            Toast.makeText(activity, "OK ! "+ firstName + " " + name + " is no longer your friend.", Toast.LENGTH_LONG).show()
            initSpinner()
            erase()
        }
    }
